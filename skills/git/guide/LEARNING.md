...
The guide improves from what a commit run finds. When a commit run turns up something
a rule would have prevented, the agent proposes a rule. The agent proposes; the developer
...
- **A recurring finding.** The same category of issue is found in more than one
  separate commit run — a one-off finding is a run result, not evidence of a gap.
- **A disagreement that reveals a wrong or missing rule.** The developer disputes a
  ...
  absent, not that the run misapplied a rule that was already right.
- **A repeated developer instruction.** The developer states the same preference more
  than once across separate commit runs or sessions.

A single finding accepted and fixed without complaint is not a gap. Most commit runs will
not reach this loop.
...
Run this after the commit or push has been reported, never before.

1. **Would a rule have prevented this for a similar commit in future?** If not, stop
   silently. One-off decisions are not rules.
2. **Is it already covered?** Search `PROCEDURE.md`, the files in `runbooks/`,
   `rules/user-rules.md`, and `rules/candidates.md`.
...
3. **Draft the rule.** One sentence, general (not tied to one commit or repository),
   actionable, not in conflict with `PROCEDURE.md` or a runbook. Attach a *why* that
   names the lesson, not the commit it came from. It always targets
   `rules/user-rules.md` — git has no other rule layer to choose between.
...
5. A new rule is in force from the **next** commit run. The developer commits and
...
with `PROCEDURE.md` or a runbook, and accompanied by a why. Proposals that fail a gate are not made.
