#!/usr/bin/env bash
# Installs the Claude Code entry point for code-better.
...
skill_link="$claude_home/skills/code-better"
if [ -d "$skill_link" ] && [ ! -L "$skill_link" ]; then
  echo "code-better: $skill_link is a real directory; remove it and re-run." >&2
  exit 1
fi
mkdir -p "$claude_home/skills"
ln -sfn "$adapter_dir" "$skill_link"

echo "code-better installed for Claude Code"
echo "  skill    $skill_link -> $adapter_dir"
echo "Invoke with the Skill tool (skill \"code-better\") in any session."
