#!/usr/bin/env bash
# Installs the Claude Code entry point for audit-content.
...
skill_link="$claude_home/skills/audit-content"
if [ -d "$skill_link" ] && [ ! -L "$skill_link" ]; then
  echo "audit-content: $skill_link is a real directory; remove it and re-run." >&2
  exit 1
fi
mkdir -p "$claude_home/skills"
ln -sfn "$adapter_dir" "$skill_link"

echo "audit-content installed for Claude Code"
echo "  skill    $skill_link -> $adapter_dir"
echo "Invoke with the Skill tool (skill \"audit-content\") in any session."
